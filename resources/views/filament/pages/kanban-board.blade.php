...
<div class="kanban-col" x-data="{ colCollapsed: localStorage.getItem('col_v1_{{ $statusKey }}_u{{ auth()->id() }}') === 'true' }"
    ...
    <div class="col-body" x-show="!colCollapsed"
        ...
        <div class="p-card {{ $cardClass }}"
             ...
             @click="if(!window._kbDragged && !$event.target.closest('button,a,input,select,label,textarea')) $wire.openEditInfoModal({{ $project->id }})"
...
{{-- LOYIHA MA'LUMOTINI TAHRIRLASH MODAL (faqat asosiy ma'lumot) --}}
        ...
        <div style="margin-bottom:16px;border:1px dashed #c7d2fe;border-radius:10px;background:#f5f7ff;padding:13px 14px">
            ...
            <div style="display:flex;gap:8px">
                <select wire:model="ei_newSvcUser" style="flex:1;padding:9px 12px;border:1.5px solid #e5e7eb;border-radius:8px;font-size:13px;outline:none;box-sizing:border-box;background:#fff">
                    ...
                    @foreach($users->whereIn('role', ['admin','menejer','bajaruvchi']) as $u)<option value="{{ $u->id }}">{{ $u->name }}</option>@endforeach
                </select>
